...
let wrongNamesCaption = document.querySelector("#wrongNamesCaption");
...
const displayWrongAnswers = () => {
  let wrongGuesses = guessedStudents.filter(
...
const displayCorrectAnswers = () => {
  let correctGuesses = guessedStudents.filter(
...
const generateStudentToGuess = () => {
  ...
  let studentAlternatives = shuffledStudents
    .slice(0, 4)
  ...
  if (e.target.tagName === "BUTTON") {
    ...
    if (amountOfGuesses < 10) {
      ...
      if (showCorrectAnswers().length === 10) {
        ...
        wrongNamesCaption.innerHTML += `<h3>Du gissade fel på</h3>`;
        displayWrongAnswers();
        rightNamesCaption.innerHTML += `<h3>Du gissade rätt på</h3>`;
